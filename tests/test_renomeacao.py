...
# ---------------------------------------------------------------------------
# Plano sequencial (VARD1, VARD2, ...)
# ---------------------------------------------------------------------------


def test_sequencial_usa_o_padrao_sem_zeros_a_esquerda(tmp_path):
    """O default e VARD1, VARD2, VARD3 - contador cru, comecando em UM."""
    caminhos = _criar_fotos(tmp_path, ["DSC1.JPG", "DSC2.jpg", "DSC3.png"])
    itens = mapear_sequencial(caminhos)

    assert [i.nome_novo for i in itens] == ["VARD1.jpg", "VARD2.jpg", "VARD3.png"]
    assert itens[0].stem_novo == "VARD1"


def test_a_contagem_comeca_em_um(tmp_path):
    """
    Nao existe VARD0: quem le o acervo conta a partir de 1, e um quadrante
    chamado "zero" so gera duvida na conferencia manual.
    """
    assert settings.SEQUENCIAL_INICIO == 1
    caminhos = _criar_fotos(tmp_path, ["unica.jpg"])
    assert mapear_sequencial(caminhos)[0].stem_novo == "VARD1"
...
def test_sequencial_nao_tem_teto_de_quantidade(tmp_path):
    ...
    assert plano.itens[0].nome_novo == "VARD1.jpg"
    assert plano.itens[-1].nome_novo == "VARD2000.jpg"
...
def test_proximo_indice_le_o_maior_ja_existente(tmp_path):
    _criar_fotos(tmp_path, ["VARD1.png", "VARD42.png", "outro.png"])
...
def test_estrategia_virtual_nao_escreve_nada(tmp_path):
    ...
    assert resultado.itens == [
        (str(origem / "IMG_1.jpg"), "VARD1"),
        (str(origem / "IMG_2.jpg"), "VARD2"),
    ]
...
def test_estrategia_mover_esvazia_a_origem(tmp_path):
    ...
    assert (destino / "VARD1.jpg").exists()
...
def test_arquivo_ilegivel_nao_derruba_a_materializacao(tmp_path):
    ...
    assert [stem for _, stem in resultado.itens] == ["VARD2"]
